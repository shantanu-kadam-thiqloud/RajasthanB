...
export default function ReportTransactionDetail() {
  ...
  const getTableData = async (values) => {
    setDataFound(false);
    const formattedData = {
      fromDate: values.fromDate ? moment(values.fromDate).format('YYYY-MM-DD') : moment().format('YYYY-MM-DD'),
      toDate: values.toDate ? moment(addDays(values.toDate, 1)).format('YYYY-MM-DD') : moment().format('YYYY-MM-DD'),
      customerName: values.customerName !== undefined ? values.customerName : "",
    ...
    if (formattedData.customerName !== "") {
      ...
      const baseUrl = "http://172.16.16.106:8080/kmbl-rsbcl-api" //process.env.REACT_APP_BASE_URL;
        await saveData(dataToSend, `${baseUrl}/eCollection_report`, (response) => {
        ...
        setDataFound(true);
          }else if(response.data && response.data.message === "No data found"){
            setRequestData([]);
            setExportData([]);
        setDataFound(false);
          }
                ...
                <div className="col-12">
                  <div className="card">
                    <div className="card-header">
                      <div className="row alignCenter">
                        <div className="col-sm-12">
                          <h1 className="m-0 pageTitle">Transaction Detail Reports</h1>
                        </div>                        
                      </div>
                    </div>
                        ...
                        <Form>
                            ...
                            <div className="col-md-2">
                                <div className="form-group">
                                    <label htmlFor="customerName">Customer Name<span className="Fieldrequired">*</span></label>
                                    <Field
                                        as="select"
                                ...
                                <div className="form-group">
                                    <label htmlFor="customerName">State</label>
                                    <Field
                                        as="select"
                                        className="form-control"
                                        id="state"
                    ...
                    <div className="wrapper text-end exportDiv">
                        <button className="btn BackBtn" onClick={exportToExcel}>Export to Excel</button>
                    </div>                    
